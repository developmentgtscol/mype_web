        ...
        if estado_header:
            ...
            if estado_token:
                ...
                if estado_json:
                    ...
                    "uid_usuario":[Required],
                    "uid_pedido":[Required],
                    "estado_pedido":[Required,In(["En tienda", "En despacho", "En reparto", "Entregado"])],
                    }
                    respuesta=validate(rules, request.json)
                    if(respuesta[0]):
                        estado_vacio,codigo_vacio=validaciones.validar_campos_vacios(request.json)
                        if estado_vacio:
                            ...
                            if estado_uid_token:
                                ...
                                if estado_permiso:
                                        pedido_model=PedidoModel()
                                        estado_registrar_pedido,codigo_registrar_pedido=pedido_model.actualizar_estado_pedido(request.json)
                                        if estado_registrar_pedido:
                                            return {'estado':estado_registrar_pedido,'mensaje':codigo_registrar_pedido}
                                        else:
                                            return {'estado':False,'codigo':codigo_registrar_pedido}
                                else:
                                     return {'estado':False,'codigo':codigo_permiso}
